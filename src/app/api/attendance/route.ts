...
const likeActivityRequestSchema = z.object({
  activityId: z.number().positive(),
  userHandle: z.string().min(1).max(50),
});

type AttendanceActivityRequest = z.infer<typeof likeActivityRequestSchema>;

export async function GET(request: NextRequest) {
  const data = await request.json();

  try {
    likeActivityRequestSchema.parse(data);
  } catch (error) {
  ...
  // that we know what we're doing and that the data is of type AttendanceActivityRequest.
  // This is safe now because we've already validated the data with zod.
  const { activityId, userHandle } = data as AttendanceActivityRequest;
  ...
  try {
    ...
    const [exist] = await db
      ...
      .where(
        and(
          eq(attendanceTable.activityId, activityId),
          eq(attendanceTable.userHandle, userHandle),
        ),
      )
      .execute();
    // The NextResponse object is a easy to use API to handle responses.
    // IMHO, it's more concise than the express API.
    return NextResponse.json({ attended: Boolean(exist) }, { status: 200 });
  } catch (error) {
